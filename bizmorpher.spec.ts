...
test.describe("Contact page", () => {
  ...
  test("displays a contact email address", async ({ page }) => {
    // Scroll to footer and target the email <a> link specifically to avoid strict mode violation
    await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
    await expect(page.getByRole("link", { name: /ask@bizmorpher\.com/i })).toBeAttached();
  });
});
